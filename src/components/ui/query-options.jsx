...
export const TagQuery = ({ icon, label, handelClick, varient, path }) => {
  const inlineStyle = `cursor-pointer transition-all text-[1.2rem] flex items-center    dark:text-black hover:scale-95      capitalize  ${
    varient === "grid-options"
      ? "p-2 px-5  rounded-3xl  gap-3 bg-gray-100 hover:bg-gray-200  "
      : "p-1 px-2 rounded-full gap-1 dark:bg-gray-300  "
  }`;
  return (
    <>
      {label === "Home" ? (
